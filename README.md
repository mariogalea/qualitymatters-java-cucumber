...
Booking is the root for business logic, thus, all actions are location under package io.qualitymatters.bdd.booking.actions.
Similar to actions, pojos which make up a response related to booking should be packaged here as well - io.qualitymatters.bdd.booking.pojo.
Finally, the utilities package are in a level higher than booking, as utilities can be used for multiple business logic.
...
The test folder holds the StepDefinitions package and the Feature Files.  Feature files are the human readable files - the tests themselves.  Everything connects to this file, which should be located in the resources folder.  Behaviours are structured as per business logic, so all features / behaviours related to booking, should be placed in the booking folder.  
